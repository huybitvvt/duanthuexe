...
| **4. Cách Ly Cache Đổi Tài Khoản & Chống Race Response Cũ** | `tests/test_user_switching_cache.py` | 1. User A (`1:all`) đăng nhập trong SPA.<br>2. Logout (`PURGE_AUTH`) làm rỗng cache và sinh `authSessionId` mới.<br>3. User B (`2:2`) đăng nhập in-place (không reload, giữ shell token), nạp cache B, assert không lộ key A.<br>4. **Real in-flight pending promise race test**: Giữ request pending của User A Session 1, logout, đăng nhập lại cùng User A (Session 2), fulfill request cũ với dữ liệu rò rỉ giả lập → assert request cũ bị hủy và không ghi đè vào cache phiên mới. | **Strict Assertions PASS**<br>Key 1:all bị xoá sạch<br>Key 2:all được tạo mới<br>In-flight pending response phiên cũ bị huỷ bỏ hoàn toàn | **ĐẠT (PASS)** |
...
| **7. Tính Toán Nghiệp Vụ, Controller & Transaction Rollback (PHPUnit)** | `.\php.cmd artisan test` | 31 tests bao gồm:<br>- Tính phụ thu trễ hạn (xeso/xega/xecon/sh theo các mốc <8h và >=8h cả ngày)<br>- Tính cọc, hoàn tiền cọc, quy tắc giá cố định handler_price<br>- `OrderController::validate_input_payment` (bắt buộc chọn ngân hàng, kiểm tra khớp tổng tiền)<br>- `OrderController::store` (chặn ngày trả trước ngày thuê)<br>- `OrderController::update` (chặn sửa hợp đồng đã thanh lý `deposit_closed`)<br>- `OrderController::complete` (chọn tài khoản ngân hàng khi hoàn cọc, bắt lỗi lệch tổng tiền, rollback transaction khi dính deadlock)<br>- `OrderController::addOnPrice` (gia hạn thuê xe, cập nhật thời hạn và số tiền gia hạn trong DB)<br>- `OrderController::calc_return_early_amount` (xử lý lỗi tính toán trả sớm)<br>- `ReceiptController::putOrPost` (bắt lỗi số tiền 0, cưỡng chế giới hạn chi nhánh `store_id = 2` của nhân viên chi nhánh, chặn inject `store_id = 999` vào DB)<br>- Phân quyền chi nhánh (Role 1 vs Role 2) và API contract | **31/31 tests PASS**<br>116 assertions OK | **ĐẠT KIỂM THỬ ĐƠN VỊ & CONTROLLER/TRANSACTION**<br>*(Chạy trên SQLite in-memory cô lập, bảo vệ tuyệt đối Supabase live)* |
...
> [!IMPORTANT]
> **Giới hạn phạm vi nghiệm thu kỹ thuật:** Các bài test PHPUnit và Live Smoke xác nhận tính toàn vẹn của logic tính toán, validation controller, transaction rollback và khả năng phản hồi của hệ thống. Các thao tác ghi đè dữ liệu kinh doanh thật (tạo đơn thật, trừ tiền thật, xóa xe) chưa được thực thi trên database Supabase live để bảo vệ dữ liệu sản xuất.
